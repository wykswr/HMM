    ...
    for (int i = 0; i < seq1.length() + 1; ++i) {
        for (int j = 0; j < seq2.length() + 1; ++j) {
            match[i][j] = match[i][j] != -1 ? match[i][j] : prob(i, j) *
                                                            (trans[1][1] * score(match, i - 1, j - 1) +
                                                             trans[2][1] * score(x_only, i - 1, j - 1) +
                                                             trans[3][1] * score(y_only, i - 1, j - 1));
            x_only[i][j] = x_only[i][j] != -1 ? x_only[i][j] : prob(i, -1) * (trans[1][2] * score(match, i - 1, j) +
                                                                              trans[2][2] * score(x_only, i - 1, j));
            y_only[i][j] = y_only[i][j] != -1 ? y_only[i][j] : prob(-1, j) * (trans[1][3] * score(match, i, j - 1) +
